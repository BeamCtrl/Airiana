...
def analyse_stat(status, user):
    ...
    if len(status) > 1:
        ...
        if (
            int(status[data["exchanger"]]) == 5
            and float(status[data["exhaust"]]) > float(status[data["supply"]])
            and float(status[data["inlet"]]) < 10
        ):
        ...
        for each in files:
            try:
                ...
                if user in list(users.keys()):
                    ...
                    if (
                        time.time() - mod
                    ) / 3600 < 48:  # send if down for 48hrs, or when returing from down
                        flag = '<font color="green">Alive  </font>'
                        if mail_sent[user] == True:
                            mail_sent[user] = False
                            mailer.setup(
                                "daniel.halling@outlook.com",
                                "airiana@outlook.com",
                                "Airiana user: "
                                + str(users[user])
                                + " has checked in and is now alive.",
                            )
                            mailer.send()
                    else:
                        ...
                        if not mail_sent[user] and not "no-mail" in sys.argv:
                            ...
                            mailer.setup(
                                ...
                                "Airiana user: "
                                + str(users[user])
                                + " has changed status to inactive.",
                            )
                            # mailer.send()
                    ...
                    if (time.time() - mod) / 3600 < 3:
                        html += (
                            '<tr><td nowrap><a href="/local_links/'
                            + each
                            + '">'
                            + users[user]
                            + "</a></td><td nowrap>"
                            + time.strftime("%d/%m %H:%M:%S", time.localtime(mod))
                            + "</td><td nowrap>"
                            + flag
                            + "</td> <td nowrap>"
                            + status_table
                        ...
                        html += (
                            '<tr style="font-style:italic"><td nowrap><a href="/local_links/'
                            + each
                            + '">'
                            + users[user]
                            + "</a></td><td nowrap>"
